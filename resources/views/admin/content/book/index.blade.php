...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            图书管理首页
            <small>Books management page</small>
        </h1>
    </section>
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach($books as $k => $v)
                                <tr>
                                    ...
                                    <td class="book-pic"><img width="100" src="{{asset('/uploads/'.$v->pic)}}" alt=""/>
                                    </td>
                                    <td>{{ strCut($v->author, 2) }}</td>
                                    <td>{{ $v->price }}</td>
                                    <td>{{ strCut($v->publisher, 2) }}</td>
                                    @if($v->name)
                                        <td>{{ $v->name}}</td>
                                        <td>{{$v->b_created_at}}</td>
                                        <td hidden>{{$endTime = strtotime($v->b_created_at) + 86400 * 30}}</td>
                                        @if(time() >= $endTime - 86400 * 7)

                                            <td style="color: #00a7d0">{{date('Y-m-d 00:00:00', $endTime)}}</td>
                                        @else
                                            <td>{{date('Y-m-d 00:00:00', $endTime)}}</td>
                                        @endif
                                    ...
                                    <td class="text-right">
                                        <div class="btn-group">
                                            ...
                                            <button type="submit" onclick="return confirm('确定删除该图书吗')"
